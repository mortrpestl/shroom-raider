...
def draw_lines(input_map, line_mode='row',points=None, element_probi=None, thickness=1, use_X_coords=False, canyonize=True):
    ...
    if line_mode == 'row': points.sort(key=lambda x:(x[0],x[1]))
    if line_mode == 'column': points.sort(key=lambda x: (x[1], x[0]))
