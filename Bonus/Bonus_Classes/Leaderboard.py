...
def show_personal_leaderboard(pdata):
    ...
    rows = [
        ...
        for i, (lvl_id, ms) in enumerate(sorted(completed.items(), key=lambda x:int(x[0])))
    ]
...
def show_general_leaderboard():
    ...
    for p in players:
        try: p["completed_levels"] = json.loads(p.get("completed_data","{}"))
        except: p["completed_levels"] = {}

    players.sort(key=lambda p:(-len(p["completed_levels"]), float(p.get("total_seconds_played") or 0)))
    rows = [
        [
            i+1,
            p.get("username",""),
    ...
    headers = ["Rank","Username","Levels Completed","Runs Played","Total Time","Mushrooms Collected","Tiles Walked"]
...
def show_level_leaderboard(level_id):
    ...
    level_rows = []
    for p in players:
        try: completed = json.loads(p.get("completed_data","{}"))
        except: completed = {}
