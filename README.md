...
- **[How It Was Made](#how-it-was-made)**
  ...
  - *[Game Logic](#game-logic)*
    ...
    - [The Grid as an R x C Grid of Stacks of Objects](#the-grid-as-an-r-x-c-grid-of-stacks-of-objects)
    - [Collision Detection](#collision-detection)
    - [Integration of Objects: Combining them All](#integration-of-objects-combining-them-all)
      - [Game Parser](#game-parser)
      - [Arg Parser](#arg-parser)
      - [Note](#note)
- **[On Testing](#on-testing)**
  - *[Running the Base Game (for Automated Testing)](#running-the-base-game-for-automated-testing)*
  - *[Why do these tests suffice?](#why-do-these-tests-suffice)*
  - *[Unit Tests](#unit-tests)*
    - [Running the Tests](#running-the-tests)
    - [Adding and Revising Tests](#adding-and-revising-tests)
    - [Example](#example)
  - *[Integrated Tests](#integrated-tests)*
    - [Running the Tests](#running-the-tests-1)
    - [Adding and Revising Tests](#adding-and-revising-tests-1)
    - [Example](#example-1)
...
Before everything, make sure to `cd Main`.

To import all necessary dependencies, also type `python3 -m pip install -r requirements.txt`

## ***Running the Base Game***

Type `python3 -m shroom_raider` and enjoy the tutorial of the game!
For running the base game in the context of testing, please see **On Testing**.
...
Your objective is to win the game. That is, collect all mushrooms without dying.

It is also, by corollary, your objective not to lose. That is, you don’t fall into the water before collecting all the mushrooms.

If you meet the conditions for either winning or losing, the game ends.

## ***Laro* 🧑**

In this game, your character is named Laro! They are denoted with a 🧑, and can do lots of things which will be delineated in the Controls section.
...
Tiles are places Laro can walk over and push blocks towards!

### **Empty Tile**

Just your basic empty tile, denoted by emptiness (if we’re being technical, it is denoted by an U+3000 ‘ideographic space’)

### **Paved Tile ⬜**

It ⬜ acts like a tile, but can be formed by pushing a rock 🪨 over water! It can be found sometimes spawned in maps, but most of the time you’ll be creating this to progress through the map in different ways.

## ***Obstacles***

### **Rock 🪨**

These rocks 🪨 are pretty heavy, but Laro is quite strong so he can push a rock to a direction if there’s nothing else immediately next to that rock in that direction!

They can also push these rocks into water, turning them less of a death trap and more of a safe paved tile ⬜ to walk through!

### **Tree 🌲**

These trees 🌲 are quite formidable… you cannot make your way past them with your bare hands.

However, if you find an axe 🪓, you can break one tree (before the axe breaks)!

Even better, if you find a flamethrower 🔥, you can burn a tree, as well as all orthogonally connected trees to it (and those also connected to that tree in the same way)!

### **Water 🟦**

Be careful! Moving Laro to water 🟦 will cause them to drown, and you will lose!

If you have a rock 🪨, however, push one to a water tile to turn them into a paved tile ⬜. This one you can walk over!

## ***Items***

All of these items are single-use. You can’t drop items, and you can’t pick up items if you have one equipped! Laro is quite the sentimental man.

### **Axe 🪓**

As stated before, you can cut down one tree with them 🪓!
An axe is used if you have it in your inventory and you move to a tree position.

### **Flamethrower 🔥**

As stated before, you can burn 🔥 down orthogonally adjacent trees!
A flamethrower is used if you have it in your inventory and you move to a tree position. All trees orthogonal to that tree and the trees orthogonal to it (and so on…) are burned.

## ***Collectables***

### **Mushroom 🍄**

There are numerous mushrooms on the map. In the name of the game, collecting them all results in victory!
...
When we were starting off, we needed a programming paradigm that:

* Will make unit testing easier in the future
* Allows ourselves to split tasks between ourselves in a way that makes sense
* Is easily extendable (makes new items, features in the future easy to add)

And object-oriented programming (OOP) ticked all of those boxes.

## ***Game Logic***

### **The Entity Class**

An entity is any element in the game that can be placed in any tile of the Grid.

Entities have different booleans for properties (all set false by default):
...
And associated methods are allowed/disallowed when a player interacts with an item with varying values for those booleans.

For example, rock has all the default values of Entity with the following exceptions:

* `is_collideable = True`
* `is_pushable = True`

You will see in the bonus that Log is almost the same, so they use similar methods from the Entity class that other classes cannot use. This way, we don’t have to make an entirely new class for every item.

We can just:

* extend the Entity class
* tweak this new subclass’ properties a little bit
* and add new methods if necessary.

### **The Grid as an R x C Grid of Stacks of Objects**

Because some items can be on top of each other (e.g. Player on PavedTile), we modeled the object representation of the Grid as an R x C Grid of Stacks.

Each stack represents a layer of the grid, the currently visible object at the top and the empty tile in the bottom.

This stack representation per coordinate makes it easier for us to:

* Detect entity collection (e.g. Axe, Mushrooms) — relevant methods can just check under the Player’s coordinate (like doing `grid[player_r][player_c][-2]`)
* Facilitate interaction of pushable objects over tiles

Meanwhile, storing the objects instead of the ASCII/Emoji representation makes it easier for us to:

* Easily choose between ASCII/Emoji for generating maps (the former is useful for testing, the latter for actual game runs)
* Use `isinstance()` to check for types (which also extends to the superclass! No need to check for multiple emojis/ASCII when you can just check if it belongs to Entity)

As you can probably observe, most of these changes are based on scalability and developer convenience.

### **Collision Detection**

*Note: For this section, we define “collisions” as two objects being in the same coordinate*

In terms of collision detection, possible collisions are checked on every “tick” or update of the game. Next, upon the detection of a collision, they are handled in a couple of different ways. In most cases, these ticks are when the player is given input, which is usually a movement command. It is because of this that collisions always happen as the result of the player moving into a position that already contains an object.

To preface, our collision detection system relies on a couple of object attributes that identify how the collisions between objects are treated. First, we have the “collideable” attribute, which basically means that that object cannot have any other collideable objects in the same tile as itself. For example, a Rock or a Tree cannot have any other object in the same tile. In these cases, the movement depends on other things such as if the player can push the rock or if the tree can be axed or burnt.

Secondly, we have non-collideable objects, such as mushrooms and items. For these objects, there are no conflicts between the player and the object, so the player is placed on top of the object in the stack (See “The Grid as an R x C grid of stacks of objects”). This is the general case for the player, however objects such as the rock have extra conditions, meaning that they cannot collide with some objects that the player can collide with. An example of this is that the player can go on top of items, while the rock cannot.

Lastly, we have a special attribute for water, which is that it is deadly. In the case of how it interacts with the player, since water is non-collideable, the player is allowed to move on top of it in its stack. However, this also causes the player to get killed in the process, deleting the player from the top of the stack. Water and Rock also have some form of this deletion interaction. In this case however, a new Paved Tile is created after Rock is “killed”, instead of it just being deleted.

### **Integration of Objects: Combining them All**

### **Game Parser**

To finish off the game logic, we must now find a way to take in input.

This is where we really utilized the power of our object-oriented implementation. We just used a parser method that takes in an input / set of inputs, and executed the game’s logic by putting that through the methods of our Player and letting it and its interactions with the objects do all the work.

### **Arg Parser**

As stated in the section “Running The Base Game”, `shroom_raider.py` can be ran with different arguments.

Using `argparse()`, these arguments are validated and ran with their respective modes as discussed in “Running The Base Game”.

### **Note**

Certain systems in place, like Test Mode (Bonus) and Integrated Tests (Bonus) also hook up their systems here!

# **On Testing**

## ***Running the Base Game (for Automated Testing)***

If you haven’t done the prerequisites yet, please do so.

Make sure to have done `cd Main` in the root directory when executing these commands.

For the commands below however, I will assume you are in the root directory of the Github repository.
...
* `python3 -m shroom_raider -f <stage_file>`
  ...
  * for playing the game normally, but with a different stage file
* `python3 -m shroom_raider -f <stage_file> -m <string_of_moves> -o <output_file>`

  * for testing, some notes below

Some notes:

* `<stage_file>` should be formatted as follows:

  * first line — space-separated R and C denoting the rows and columns in the stage
  * next r lines – stage file, containing valid stages in ASCII form
* `<string_of_moves>`
* `<output_file>` is the name of the file you would want the result of the end game after the string of moves is performed on the stage. It is formatted as follows:

  * first line — space-separated R and C denoting the rows and columns in the stage
  * second line — a string `NO CLEAR` or `CLEAR`, depending on whether the player has not won or won respectively.
  * next r lines – stage file after string of moves, containing valid stages in ASCII form

## ***Why do these tests suffice?***

Our tenets for test creation are as follows:

A test must:

* Should prioritize depth over breadth. Only test for one or a small set of relatively specific scenarios.
* Have names and error messages that are readable and descriptive.
* Not test for something that another test has also tested for.

With this in mind, we:

* Created many small yet deep tests targeting different areas of the game logic
* Split our tests into two: the unit tests and integrated tests. More on those below.

Through the following, we believe that our tests are more than enough (while still offering an easy way to add more) to ensure the integrity of our game logic:

* reasonable quantity of the tests (~50 for unit tests, ~75 for integrated tests, with a total of ~130 tests)
* reasonable depth of each test (run `python3 -m pytest` and let the descriptions speak for themselves)
* easy scalability of our tests (adding tests are made easy through separating testing files in Unit tests, and devising an Excel system for adding/editing integrated tests)

## ***Unit Tests***

These tests are a way to check if the class and instance functions of each class in the Game work properly.

We have a total of **50** unit tests, following the testing principles we mentioned.

### **Running the Tests**

The tests can be ran with the following command:
`python3 -m pytest "Main/Tests/Unit Tests"`

### **Adding and Revising Tests**

This is made easy by separating each class’ unit tests into their own files!

Note that most of the imports (from other classes) we expect to be used in each class has also been imported, for the testers’ convenience.

1. Open “Main/Tests/Unit Tests”
2. Add a function starting with `test_` and the assert you want
3. Re-run the tests!

![](md_images/pytest_unit.png)

### **Example**

The test below simulates the conversion of Water to PavedTile from a Rock interaction. This test meets our criterion of specificity. This is 1 of 5 tests under Rock.
...
These tests are a way to check how well the game’s elements mesh together! Combined with the fact that integrated tests allow automated testing of how a user actually uses the game (through inputs), we would say this is a very important phase of testing.

We have a total of **77** integrated tests, following the testing principles we mentioned.

### **Running the Tests**

The tests can be ran with the following command:
`python3 -m pytest "Main/Tests/Integrated Tests/test_Integrated.py"`

![](md_images/pytest2.png)

### **Adding and Revising Tests**

This is made easy using the Excel system for adding tests! (**Logged for bonus**)

1. Open `integrated_tests.xlsx` in “Main/Tests/Integrated Tests”
2. Fill in the columns:

   * ID — unique identifier for the test
   * Input Grid — specified with `R C` in the first line and a valid R x C grid in the next R lines
   * Input String — the string of moves to be made. Create multiline strings to simulate consecutive outputs from player (helpful for testing invalid moves)
   * Output — the expected output given the test. If it doesn’t match, the tester will throw an AssertionError
3. You may also fill in Remarks and Date Added if you want to leave notes for future references.

### **Example**

The test below simulates a game where the Player repeatedly attempts to pick up an item at an empty tile. This test actually allowed us to fix one of the last bugs in our code.

![image1](md_images/integ_tests.png)

***---The contents below now belong to BONUS---***
...
* **[DLC: The Bigger Picture](#dlc-the-bigger-picture)**
  ...
  * *[Level Design](#level-design)*
    ...
    * [Game](#game)
    * [A Glimpse of a Level Designer’s Mind](#a-glimpse-of-a-level-designers-mind)
  * *[Graphics](#graphics)*
...
* Check if you are in Python 3.11. Our version of `pygame` only works in Python 3.11 (actually, you can try running it anyway, that problem might just be on our devices' end)
...
![image1](md_images/map_gen.png)

Drawing a pentagonal beehive!

![](md_images/polygon.png)
