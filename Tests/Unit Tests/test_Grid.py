...
def test_render_behavior_returns_expected_values(small_grid):
    ...
    g = small_grid
    assert g.render(DummyPlayer(), g, "", None, test_mode=True) is None
    assert g.render(WinPlayer(g), g, "", None, test_mode=True) == 'reset_only'
    assert g.render(LosePlayer(), g, "", None, test_mode=True) == 'reset_only'
